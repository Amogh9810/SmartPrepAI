...
-- Core tables for managing subjects, topics, quiz questions, and study tracking

-- 1. SUBJECTS TABLE
-- Stores subject information (e.g., Math, Physics, Chemistry)
...
-- 2. TOPICS TABLE
-- Stores topics extracted from syllabus for each subject
...
DROP POLICY IF EXISTS "topics_select_own_subjects" ON public.topics;
DROP POLICY IF EXISTS "topics_insert_own_subjects" ON public.topics;
DROP POLICY IF EXISTS "topics_update_own_subjects" ON public.topics;
DROP POLICY IF EXISTS "topics_delete_own_subjects" ON public.topics;

CREATE POLICY "topics_select_own_subjects" ON public.topics 
  FOR SELECT USING (
    subject_id IN (
      SELECT id FROM public.subjects WHERE auth.uid() = user_id
    )
  );
CREATE POLICY "topics_insert_own_subjects" ON public.topics 
  FOR INSERT WITH CHECK (
    subject_id IN (
      SELECT id FROM public.subjects WHERE auth.uid() = user_id
    )
  );
CREATE POLICY "topics_update_own_subjects" ON public.topics 
  FOR UPDATE USING (
    subject_id IN (
      SELECT id FROM public.subjects WHERE auth.uid() = user_id
    )
  );
CREATE POLICY "topics_delete_own_subjects" ON public.topics 
  FOR DELETE USING (
    subject_id IN (
      SELECT id FROM public.subjects WHERE auth.uid() = user_id
    )
...
-- 3. QUESTIONS TABLE
-- Stores quiz questions for each topic
...
DROP POLICY IF EXISTS "questions_select_own_topics" ON public.questions;
DROP POLICY IF EXISTS "questions_insert_own_topics" ON public.questions;
DROP POLICY IF EXISTS "questions_update_own_topics" ON public.questions;
DROP POLICY IF EXISTS "questions_delete_own_topics" ON public.questions;

CREATE POLICY "questions_select_own_topics" ON public.questions 
  FOR SELECT USING (
    topic_id IN (
...
CREATE POLICY "questions_insert_own_topics" ON public.questions 
  FOR INSERT WITH CHECK (
    topic_id IN (
      SELECT id FROM public.topics 
      WHERE subject_id IN (
...
CREATE POLICY "questions_update_own_topics" ON public.questions 
  FOR UPDATE USING (
    topic_id IN (
      SELECT id FROM public.topics 
      WHERE subject_id IN (
...
CREATE POLICY "questions_delete_own_topics" ON public.questions 
  FOR DELETE USING (
    topic_id IN (
      SELECT id FROM public.topics 
      WHERE subject_id IN (
...
-- Tracks quiz attempts and performance
...
-- Tracks user study sessions for time management and analytics
...
-- 6. SCHEDULES TABLE
-- Stores AI-generated study schedules
